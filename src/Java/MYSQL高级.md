...
[MySQL 数据库入门到大牛，mysql 安装到优化，百科全书级，全网天花板](https://www.bilibili.com/video/BV1iq4y1u7vj?p=187&vd_source=3c93d521158d3aa4f74c71c5140ba8dc)

[MySQL 是怎样运行的：从根儿上理解 MySQL](https://juejin.cn/book/6844733769996304392?enter_from=search_result&utm_source=search)

[MySQL 实战 45 讲](https://time.geekbang.org/column/intro/100020801?utm_campaign=geektime_search&utm_content=geektime_search&utm_medium=geektime_search&utm_source=geektime_search&utm_term=geektime_search)
...
![聚簇索引结构 2](https://img.leftover.cn/img-md/202408271644061.png)
...
1. 插入的 3 种模式：
   ...
   >批量插入：不确定要插入的行数
   >
   >混合模式插入：是 simple insert 的语句，但是只需要指定部分新行的自动递增值 [MYSQL 高级.md](MYSQL高级.md) 
...
这部分需要画图才能更好地理解。详情请看 [READ COMMITTED —— 每次读取数据前都生成一个 ReadView](https://relph1119.github.io/mysql-learning-notes/#/mysql/24-一条记录的多幅面孔-事务的隔离级别与MVCC?id=read-committed-每次读取数据前都生成一个readview)



### RR 隔离级别下

详情请看 [REPEATABLE READ —— 在第一次读取数据时生成一个 ReadView](https://relph1119.github.io/mysql-learning-notes/#/mysql/24-一条记录的多幅面孔-事务的隔离级别与MVCC?id=repeatable-read-在第一次读取数据时生成一个readview)
...
3. 根据业务的实际情况调整 mysql 的参数配置（具体查看 [优化 MySQL 的配置的场景参数](#优化mysql的配置的场景参数)）
...
物理备份比较复杂，可以使用专门的工具，例如 [xtrabackup](https://www.percona.com/mysql/software/percona-xtrabackup), 只支持 Linux
...
可以使用一些 [flashback 工具](https://juejin.cn/post/6950641397695250439#heading-0) 进行回滚，本质上是通过对 bin log 的分析进行一个回滚，例如美团开源的 [MyFlash](https://github.com/Meituan-Dianping/MyFlash)，[binlog2sql](https://github.com/danfengcao/binlog2sql) 可以将 binlog 转为 sql 再进行一个手动的重放。其他工具可以查看 [MySQL Flashback 工具介绍](https://juejin.cn/post/6950641397695250439#heading-2)
